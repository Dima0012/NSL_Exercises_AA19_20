...
int main()
{ 
  int step = 20; //Values of T
  ReadTemp.open("temp.dat");

  for(int i = 0; i<step; i++){  //Cycle to runn all simulations needed
...
//Read input informations
  ...
  filenumber = to_string(temp_number);
...
//initial configuration
  ...
  if(restart==0){
    ...
    ifstream res("config.final");  //Reading config from file
    for (int i=0; i<nspin; ++i)
    {
      res >> s[i];
    }
