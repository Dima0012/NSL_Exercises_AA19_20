...
void population::data_print(string filename)
{
	Sort();
...
//Print best path
void population::best_print(string filename)
{
	Sort();
...
vector<int> population::get_path(int pos)
{
	Sort();
	return m_population[pos].get_path();

}

void population::set_path(int pos, vector<int> path)
{
	Sort();
	individual new_path(m_x, m_y, path);
	m_population[pos] = new_path;
	Sort();
}